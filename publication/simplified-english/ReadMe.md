...
# The simplified-English edition
...
substance, without the academic vocabulary and the assumed references.
    ...
    contents/english/             the upstream text, unchanged
      ↓  adapted to the reader's education and vocabulary, not to another language
    contents/simplified-english/  this edition

It is not a summary or an abridgement. A paragraph that explains a term the reader lacks
is longer than one that assumed it, and the checker enforces that: a chapter that comes
out much shorter than its source has lost something rather than tightened it.

## This branch is generated — do not commit to it

`simplify` is a publication branch, like a `dist` or `gh-pages` branch. It is rebuilt from
the `norwegian` branch of this repository, which carries this layer together with the
Norwegian translation that is made from it. A commit made here is discarded by the next
rebuild.

Work on the simplified English happens on `norwegian`, one chapter to a branch:
...
Who this edition is written for, and the rules of register that follow from it, are in
[docs/translation/audience.md](docs/translation/audience.md), with one note per chapter in
[docs/translation/audience/](docs/translation/audience/). The stage itself is described in
[docs/translation/simplify.md](docs/translation/simplify.md), and contributors working
through a chat assistant with no repository access can use the ready-made prompt in
[docs/translation/prompts/simplify.md](docs/translation/prompts/simplify.md). Check a
chapter before opening a pull request with:

```bash
vp run translation:validate
```

The registries under [translation/](translation/) are shared with the Norwegian edition,
so `translation/chapter-titles.tsv` carries a Norwegian title for every chapter and
`translation/glossary.tsv` a Norwegian term for many. Nothing on this branch reads them;
the glossary matters here because the English side of it is the terminology this edition
